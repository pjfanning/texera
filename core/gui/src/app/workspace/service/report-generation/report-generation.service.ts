...
export class ReportGenerationService {
    ...
    return new Promise(async (resolve, reject) => {
      this.checkAIAssistantEnabled().then(async AIEnabled => {
        ...
        try {
          ...
          this.generateComment(operatorDetails)
            .then(comment => {
   ...
   * @returns {void}
   */
  public async generateReportAsHtml(
    ...
    while (attempts <= maxRetries) {
      try {
        const response = await firstValueFrom(
          this.http.post<any>(`${AI_ASSISTANT_API_BASE_URL}/aiassistant/generateComment`, { prompt }).pipe(
            map(response => {
  ...
  public async generateSummaryComment(operatorInfo: any): Promise<string> {
    ...
    while (attempts <= maxRetries) {
      try {
        const response = await firstValueFrom(
          this.http.post<any>(`${AI_ASSISTANT_API_BASE_URL}/aiassistant/generateSummaryComment`, { prompt }).pipe(
            map(response => {
              const content = response.choices[0]?.message?.content.trim() || "";
              return content;
            })
