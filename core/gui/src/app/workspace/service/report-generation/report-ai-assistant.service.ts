...
export class ReportAiAssistantService {
  ...
  public checkAIAssistantEnabled(): Observable<boolean> {
    if (this.isAIAssistantEnabled !== null) {
      return of(this.isAIAssistantEnabled);  // Return an Observable of the cached value
    }
  ...
  public generateComment(operatorInfo: any): Observable<string> {
    ...
    return new Observable<string>(observer => {
      ...
      this.checkAIAssistantEnabled().subscribe((AIEnabled: boolean) => {
        if (!AIEnabled) {
          ...
          const tryRequest = () => {
            this.http.post<any>(`${AI_ASSISTANT_API_BASE_URL}/aiassistant/generateComment`, { prompt })
              .pipe(
                map(response => {
    ...
    return new Observable<string>(observer => {
      ...
      this.checkAIAssistantEnabled().subscribe((AIEnabled: boolean) => {
        if (!AIEnabled) {
          ...
          // Retry logic for up to maxRetries attempts
          const tryRequest = () => {
            this.http.post<any>(`${AI_ASSISTANT_API_BASE_URL}/aiassistant/generateSummaryComment`, { prompt })
              .pipe(
                map(response => {
                  const content = response.choices[0]?.message?.content.trim() || "";
                  return content;
