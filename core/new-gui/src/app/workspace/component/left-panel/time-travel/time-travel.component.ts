...
export class TimeTravelComponent implements OnInit, OnDestroy {
  ...
  onInteractionClick(vid: number, eid: number, envId: number, interaction: string) {
    let wid = this.getWid();
    if (wid === undefined) {
      return;
    }
    this.workflowVersionService
      ...
      .subscribe(workflow => {
        ...
        this.executeWorkflowService.executeWorkflowAmberTexeraWithReplay(envId, replayExecutionInfo);
      });
  }
