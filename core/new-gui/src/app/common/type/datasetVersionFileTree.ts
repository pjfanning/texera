    ...
    private removeNodeAndDescendants(node: DatasetVersionFileTreeNode): void {
        if (node.type === 'directory' && node.children) {
            node.children.forEach(child => {
                const childPath = node.parentDir === '/' ? `/${node.name}/${child.name}` : `${node.parentDir}/${node.name}/${child.name}`;
...
export function parseFileUploadItemToVersionFileTreeNodes(fileUploadItems: FileUploadItem[]): DatasetVersionFileTreeNode[] {
