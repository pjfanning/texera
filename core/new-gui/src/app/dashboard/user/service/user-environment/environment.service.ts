...
export class EnvironmentService {
  ...
  retrieveEnvironmentByEid(eid: number): Observable<DashboardEnvironment> {
    return this.http.get<DashboardEnvironment>(`${AppSettings.getApiEndpoint()}/${ENVIRONMENT_BASE_URL}/${eid}`).pipe(
  ...
  linkWorkflowToEnvironment(wid: number, eid: number): Observable<Response> {
    return this.http.post<Response>(
      `${AppSettings.getApiEndpoint()}/${ENVIRONMENT_BASE_URL}/${eid}/${ENVIRONMENT_LINK_WORKFLOW}`,
      {
        wid: wid,
      }
    );
