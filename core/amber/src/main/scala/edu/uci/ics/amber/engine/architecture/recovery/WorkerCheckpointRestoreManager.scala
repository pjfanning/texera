...
class WorkerCheckpointRestoreManager(@transient worker: WorkflowWorker)
    extends CheckpointRestoreManager(worker) {
  ...
  def onCheckpointCompleted(pendingCheckpoint: PendingCheckpoint): Unit = {
    worker.initiateSyncActionFromMain(() => {
      ...
      worker.dataProcessor.outputPort.sendToClient(
        RuntimeCheckpointCompleted(
          actorId,
  ...
  override def setupReplay(
      replayId: String,
      logReader: DeterminantLogReader,
    ...
    worker.internalQueue =
      new RecoveryInternalQueueImpl(worker.actorId, worker.creditMonitor, replayOrderEnforcer)
    replayOrderEnforcer
  }
  ...
  override def doCheckpointDuringReplay(
  ...
  ): () => Unit = { () =>
    {
      ...
      worker.initiateSyncActionFromDP(() => {
        logger.info(
          s"taking checkpoint during replay at step ${worker.dataProcessor.cursor.getStep}"
        )
        worker.dataProcessor.outputPort.broadcastMarker(
        ...
        logger.info(
          s"state serialization done, pending checkpoint aligned = ${pendingCheckpoint.isNoLongerPending}"
        )
  ...
  override def startProcessing(
    ...
    worker.dpThread =
      new DPThread(actorId, worker.dataProcessor, worker.internalQueue, worker, replayOrderEnforcer)
