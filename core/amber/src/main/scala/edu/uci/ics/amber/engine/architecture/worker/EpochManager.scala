...
class EpochManager(inputGateway: InputGateway, val actorId: ActorVirtualIdentity) extends AmberLogging {
  ...
  def isMarkerAligned: Boolean = {
    ...
    val markerReceivedFromAllChannels = getChannelsWithinScope.subsetOf(epochMarkerReceived(markerId))
  ...
  def getChannelsWithinScope: Set[ChannelID] = {
    ...
    inputGateway.getAllChannels.map(_.channelId).filter {
      id =>
