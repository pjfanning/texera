...
import edu.uci.ics.amber.engine.common.ambermessage.{ChannelEndpointID, ControlPayload, IdempotentInternalPayload, MarkerAlignmentInternalPayload, MarkerCollectionSupport, NeverCompleteMarkerCollection, OneTimeInternalPayload, WorkflowFIFOMessage}
...
class ControllerInternalPayloadManager(controller:Controller) extends InternalPayloadManager with AmberLogging{
  ...
  override def markerAlignmentStart(channel: ChannelEndpointID, markerAlignmentInternalPayload: MarkerAlignmentInternalPayload, pendingCollections:mutable.HashMap[String, MarkerCollectionSupport]): Unit = {
    markerAlignmentInternalPayload match {
      case TakeRuntimeGlobalCheckpoint(id, _, _) =>
