  ...
  override def generatePythonCode(): String = {
    ...
    val finalcode =
      s"""
         ...
         |          model = SVR(kernel=kernal_value,C=float(c_value),gamma=gamma_value,coef0=float(coef_value))
         |        elif kernal_value in ['rbf']:
         |          gamma_value = gamma_list[i]
         |          para_str = "kernal_value = '{}';c_value= {};gamma_value= {}".format(kernal_value,c_value,gamma_value)
         |          model = SVR(kernel=kernal_value,C=float(c_value),gamma=gamma_value)
         ...
         |          model = SVR(kernel=kernal_value,C=float(c_value))
