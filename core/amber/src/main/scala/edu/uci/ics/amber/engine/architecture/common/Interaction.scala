...
class Interaction {
  ...
  def addParticipant(id:ActorVirtualIdentity, alignment:Long, checkpointCost: Long, loadCost:Long):Unit = {
  ...
  def getCheckpointCost(id: ActorVirtualIdentity):Option[Long] = participants.get(id).map(x => checkpointCosts(x))

  def getParticipants: Iterable[ActorVirtualIdentity] = participants.keys

  def getAlignment(id: ActorVirtualIdentity):Option[Long] = participants.get(id).map(x => alignments(x))

  def getLoadCost(id: ActorVirtualIdentity): Option[Long] = participants.get(id).map(x => loadCosts(x))

  def getTotalLoadCost: Long = loadCosts.sum

  def getAlignmentMap:Map[ActorVirtualIdentity, Long] = participants.map(x => x._1 -> alignments(x._2)).toMap
