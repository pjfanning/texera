...
class DatasetFileDesc(val fileName: Path, val datasetPath: Path, val versionHash: String) {
  def tempFilePath(): Path = {
    GitVersionControlLocalFileStorage.writeVersionedFileToTempFile(datasetPath, versionHash, datasetPath.resolve(fileName))
  }

  override def toString: String = s"DatasetFileDesc(fileName=$fileName, datasetPath=$datasetPath, versionHash=$versionHash)"
}