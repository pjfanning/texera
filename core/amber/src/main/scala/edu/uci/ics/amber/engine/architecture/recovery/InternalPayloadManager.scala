...
object InternalPayloadManager {
  ...
  // replay related:
  case class LoadStateAndReplay(
      id: String,
      checkpointStep: Option[Long],
      replayTo: Option[Long],
      checkpointConfigs: Array[ReplayCheckpointConfig]
  ) extends OneTimeInternalPayload
  case class ContinueReplay(
      id: String,
      replayTo: Option[Long],
      checkpointConfigs: Array[ReplayCheckpointConfig]
  ) extends OneTimeInternalPayload
  ...
  def setupLoggingForWorkflowActor(actor: WorkflowActor, loggingEnabled: Boolean): Unit = {
    if (loggingEnabled) {
      ...
      actor.logManager =
        new LogManagerImpl(actor.networkCommunicationActor, actor.determinantLogger)
  ...
  def retrieveLogForWorkflowActor(
      actor: WorkflowActor
  ): DeterminantLogStorage.DeterminantLogReader = {
    new LocalFSLogStorage(actor.getLogName).getReader
  }

  // checkpoint related:
  case class TakeRuntimeGlobalCheckpoint(
      id: String,
      alignmentMap: Map[ActorVirtualIdentity, Set[ChannelEndpointID]]
  ) extends MarkerAlignmentInternalPayload

  // controller replay workflow:
  case class ReplayWorkflow(id: String, replayConfig: WorkflowReplayConfig)
      extends OneTimeInternalPayload
...
abstract class InternalPayloadManager {
  ...
  protected val pending: mutable.HashMap[String, MarkerCollectionSupport] =
    mutable.HashMap[String, MarkerCollectionSupport]()
