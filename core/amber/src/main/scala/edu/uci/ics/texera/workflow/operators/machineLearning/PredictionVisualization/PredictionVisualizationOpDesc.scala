...
class PredictionVisualizationOpDesc extends PythonOperatorDescriptor {
  ...
  override def getOutputSchema(schemas: Array[Schema]): Schema = {
    ...
    outputSchemaBuilder.add(new Attribute("Iteration", AttributeType.DOUBLE))
    outputSchemaBuilder.add(new Attribute(predictValue, AttributeType.STRING))
    if (is_prob){
    var classList = this.generateClassStrings(class_number)
