...
/**
  ...
  * e.g. for dataset file, supports for read/write using file stream are essential as the dataset file, whereas read & write using index are hard to support and are semantically meaningless
