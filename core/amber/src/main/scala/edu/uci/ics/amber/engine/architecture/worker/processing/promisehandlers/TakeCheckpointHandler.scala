    ...
    dp.operator match {
      case support: CheckpointSupport =>
        if(!CheckpointHolder.hasMarkedCompletion(actorId, dp.totalValidStep)){
          dp.outputIterator.setTupleOutput(support.serializeState(dp.outputIterator.outputIter, msg.chkpt))
        }
      case _ =>
    }
    msg.chkpt.save("controlState", dp)
    msg.chkpt.save(
      "outputMessages", dp.logManager.getUnackedMessages()
    )
    // push to storage
    CheckpointHolder.addCheckpoint(actorId, dp.totalValidStep, msg.chkpt, dp.stateManager.getCurrentState == COMPLETED)
    logger.info(
