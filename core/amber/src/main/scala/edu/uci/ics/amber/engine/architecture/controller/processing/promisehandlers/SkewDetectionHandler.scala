...
import edu.uci.ics.amber.engine.architecture.worker.processing.promisehandlers.SendImmutableStateOrNotifyHelperHandler.SendImmutableStateOrNotifyHelper
...
object SkewDetectionHandler {
  ...
  def getPreviousWorkerLayer(
    ...
    if (workflow.getOperator(opId).opExecClass == classOf[HashJoinOpExec[_]]) {
      ...
      execution.getOperatorExecution(upstreamLayers(0).id)
    }
  }
...
trait SkewDetectionHandler {
  ...
  registerHandler((msg: ControllerInitiateSkewDetection, sender) => {
    if (
      ...
      cp.execution.getAllOperatorExecutions.foreach {
        case (id, opExecution) => {
          if (
            ...
            val prevWorkerLayer = getPreviousWorkerLayer(id, cp.workflow, cp.execution)
