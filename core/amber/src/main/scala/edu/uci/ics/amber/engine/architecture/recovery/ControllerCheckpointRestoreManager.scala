...
class ControllerCheckpointRestoreManager(@transient controller: Controller)
    extends CheckpointRestoreManager(controller) {
  ...
  override def onCheckpointCompleted(pendingCheckpoint: PendingCheckpoint): Unit = {
    ...
    controller.controlProcessor.outputPort.sendToClient(
      RuntimeCheckpointCompleted(
        actorId,
        pendingCheckpoint.logicalSnapshotId,
  ...
  def killAllExistingWorkers(): Unit = {
    ...
    controller.controlProcessor.execution.getAllWorkers.foreach { worker =>
      val workerRef =
        controller.controlProcessor.execution.getOperatorExecution(worker).getWorkerInfo(worker).ref
      workerRef ! PoisonPill
    }
  }

  def restoreWorkers(): Unit = {
    Await.result(Future.collect(controller.controlProcessor.execution.getAllWorkers.map { worker =>
  ...
  override def setupReplay(
      replayId: String,
      logReader: DeterminantLogStorage.DeterminantLogReader,
      replayTo: Option[Long]
  ): ReplayOrderEnforcer = {
    val replayOrderEnforcer = new ReplayOrderEnforcer(
      logReader.getLogs[StepsOnChannel],
      () => {
    ...
    controller.replayQueue = new ControllerReplayQueue(
      controller.controlProcessor,
      replayOrderEnforcer,
      controller.controlProcessor.processControlPayload
    )
    replayOrderEnforcer
  }
  ...
  override def doCheckpointDuringReplay(
      pendingCheckpoint: PendingCheckpoint,
      conf: ReplayCheckpointConfig
  ): () => Unit = { () =>
    {
      controller.controlProcessor.outputPort.broadcastMarker(
        TakeRuntimeGlobalCheckpoint(conf.checkpointId, Map.empty)
      )
  ...
  override def startProcessing(
    ...
    logger.info(
      s"controller restored! output Seq: ${controller.controlProcessor.outputPort.getFIFOState}"
    )
  ...
  override protected def replayCompletedCallback(replayId: String): () => Unit = { () =>
    {
      ...
      controller.controlProcessor.asyncRPCClient.sendToClient(
        WorkflowStatusUpdate(controller.controlProcessor.execution.getWorkflowStatus)
      )
      controller.controlProcessor.outputPort.sendToClient(ReplayCompleted(actorId, replayId))
    }
  }
