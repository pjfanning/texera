...
object LogicalExecutionSnapshot {
  ...
  final case class ProcessingStats(
      checkpointCost: Long,
      alignment: Long,
      inputStatus: ChannelStatsMapping
  )
}

class LogicalExecutionSnapshot(val id: String, val isInteraction: Boolean, val timestamp: Long)
  ...
  def addParticipant(
    ...
    participants(id) =
      cur.copy(checkpointCost = info.saveStateCost + info.alignmentCost, alignment = info.step)
    if (isCheckpointed) {
      ...
      info.outputWatermarks.foreach {
        case (channel, sent) =>
          val down = participants.getOrElseUpdate(
            channel.endpointWorker,
            ProcessingStats(0, 0, new ChannelStatsMapping())
          )
