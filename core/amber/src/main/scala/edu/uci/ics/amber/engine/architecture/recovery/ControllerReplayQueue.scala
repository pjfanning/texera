...
class ControllerReplayQueue(
    @transient controlProcessor: ControlProcessor,
    @transient replayOrderEnforcer: ReplayOrderEnforcer,
    @transient processPayload: (ChannelEndpointID, ControlPayload) => Unit
    ...
    while (continue) {
      ...
      if (replayOrderEnforcer.isReplayCompleted(currentStep)) {
        ...
        if (replayOrderEnforcer.isPayloadRecorded) {
          processPayload(
            currentChannel,
            replayOrderEnforcer.getRecordedPayload.asInstanceOf[ControlPayload]
          )
        } else {
          if (messageQueues.getOrElseUpdate(currentChannel, new mutable.Queue()).nonEmpty) {
            ...
            println(
              s"reprocessing message from channel = $currentChannel content = $controlPayload at step = $currentStep"
            )
