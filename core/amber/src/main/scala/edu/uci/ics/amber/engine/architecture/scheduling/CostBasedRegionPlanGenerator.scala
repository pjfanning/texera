...
class CostBasedRegionPlanGenerator(
  ...
  private def createRegionDAG(): DirectedAcyclicGraph[Region, RegionLink] = {
    ...
    val searchResult = Try(Await.result(searchResultFuture, AmberConfig.searchTimeoutMilliseconds.milliseconds)) match {
      case Failure(exception) =>
        logger.info("Search for region plan timed out, falling back to top-down greedy search:", exception)
