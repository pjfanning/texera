...
sealed abstract class ConsoleMessageType(val value: _root_.scala.Int) extends _root_.scalapb.GeneratedEnum {
  ...
  def companion: _root_.scalapb.GeneratedEnumCompanion[ConsoleMessageType] = edu.uci.ics.amber.engine.architecture.rpc.ConsoleMessageType
  final def asRecognized: _root_.scala.Option[edu.uci.ics.amber.engine.architecture.rpc.ConsoleMessageType.Recognized] = if (isUnrecognized) _root_.scala.None else _root_.scala.Some(this.asInstanceOf[edu.uci.ics.amber.engine.architecture.rpc.ConsoleMessageType.Recognized])
}
