...
class DataProcessor(Runnable):
    def __init__(self, input_queue: Queue, output_queue: Queue,
                 operator: Optional[Operator], dp_condition):
    ...
    def run(self) -> None:
        ...
        while self._running.is_set():
            ...
            for output in self._operator.get().process_tuple(tuple_, input_) if \
                    isinstance(
                    tuple_, Tuple) else self._operator.get().on_finish(input_):
