...
class VersionControlledStorageSpec extends AnyFlatSpecLike with BeforeAndAfterAll {
  ...
  private def initVersionStore(repoURI: TexeraURI): Unit = {
    val versionRepo: VersionControlledCollection = new GitVersionControlledCollection(repoURI, repoURI, None)
    versionRepo.initVersionStore()
  }

  private def createVersionByAddOneFile(repoURI: TexeraURI, fileURI: TexeraURI, versionName: String, fileContent: String): String = {
    val versionRepo: VersionControlledCollection = new GitVersionControlledCollection(repoURI, repoURI, None)
  ...
  private def createVersionByDeleteOneFile(repoURI: TexeraURI, fileURI: TexeraURI, versionName: String): String = {
    val versionRepo: VersionControlledCollection = new GitVersionControlledCollection(repoURI, repoURI, None)
    versionRepo.withCreateVersion(versionName)({
      val doc = new GitVersionControlledDocument(repoURI, fileURI, None)
      doc.rm()
  ...
  private def retrieveFileContentOfOneVersion(repoURI: TexeraURI, fileURI: TexeraURI, commitHash: String): String = {
    val document: TexeraDocument[_] = new GitVersionControlledDocument(repoURI, fileURI, Some(commitHash))
...
//    testRepoPath = Files.createTempDirectory("testRepo")
    testRepoPath = Paths.get("/Users/baijiadong/Desktop/chenlab/texera/core/amber/user-resources/datasets/testDir")
    testRepoURI = TexeraURI.apply(FILE_SCHEMA, testRepoPath.toString)

    initVersionStore(testRepoURI)

    val fileURI = TexeraURI.apply(FILE_SCHEMA, testRepoPath.resolve(testFile1Name).toString)
  ...
  override def afterAll(): Unit = {
    ...
    val testRepo: VersionControlledCollection = new GitVersionControlledCollection(testRepoURI, testRepoURI, None)
    testRepo.rm()
  }

  "File content" should "match across versions" in {
    val filePath = testRepoPath.resolve(testFile1Name)
    val fileURI: TexeraURI = TexeraURI.apply(FILE_SCHEMA, filePath.toString)

    retrieveFileContentOfOneVersion(testRepoURI, fileURI, testRepoMasterCommitHashes.head) should be (testFile1ContentV1)
    retrieveFileContentOfOneVersion(testRepoURI, fileURI, testRepoMasterCommitHashes(1)) should be (testFile1ContentV2)
    retrieveFileContentOfOneVersion(testRepoURI, fileURI, testRepoMasterCommitHashes(2)) should be (testFile1ContentV3)
  }
  ...
  "File tree nodes" should "be consistent" in {
    ...
    val testRepoV3: VersionControlledCollection = new GitVersionControlledCollection(testRepoURI, testRepoURI, Some(testRepoMasterCommitHashes.last))
    ...
    val file2URI = TexeraURI.apply(FILE_SCHEMA, file2Path.toString)
    ...
    val testRepoV4: VersionControlledCollection = new GitVersionControlledCollection(testRepoURI, testRepoURI, Some(testRepoMasterCommitHashes.last))
    ...
    val testRepoV5: VersionControlledCollection = new GitVersionControlledCollection(testRepoURI, testRepoURI, Some(testRepoMasterCommitHashes.last))
