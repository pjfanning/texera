...
//import edu.uci.ics.amber.engine.architecture.worker.processing.{BreakpointManager, DataProcessor, UpstreamLinkStatus}
//import edu.uci.ics.amber.engine.architecture.worker.statistics.WorkerState.{COMPLETED, UNINITIALIZED}
//import edu.uci.ics.amber.engine.common.ambermessage.{ChannelEndpointID, ControlPayload, DPMessage, DataFrame, DataPayload, EndOfUpstream, WorkflowFIFOMessage, WorkflowFIFOMessagePayload, WorkflowMessage}
...
//  lazy val mockInputHandler: (ChannelEndpointID, WorkflowFIFOMessagePayload) => Unit =
//    mock[(ChannelEndpointID, WorkflowFIFOMessagePayload) => Unit]
//  lazy val mockOutputHandler
//      : (ActorVirtualIdentity, WorkflowMessage) => Unit =
//    mock[(ActorVirtualIdentity, WorkflowMessage) => Unit]
//  lazy val mockDataInputPort: NetworkInputPort =
//    new NetworkInputPort(identifier, mockInputHandler)
//  lazy val mockDataOutputPort: NetworkOutputPort =
//    new NetworkOutputPort(identifier, mockOutputHandler)
...
//  ): Future[_] = {
//    Future {
//      val channelId = ChannelEndpointID(senderWorker, false)
//      data.foreach { x =>
//        dp.internalQueue.enqueuePayload(DPMessage(channelId, DataFrame(Array(x))))
//        if (interval > 0) {
//          Thread.sleep(interval)
//        }
//      }
//      dp.internalQueue.enqueuePayload(DPMessage(channelId, EndOfUpstream()))
...
//  ): Future[_] = {
//    Future {
//      val channelId = ChannelEndpointID(CONTROLLER, true)
//      control.foreach { x =>
//        dp.internalQueue.enqueuePayload(DPMessage(channelId, x))
//        if (interval > 0) {
//          Thread.sleep(interval)
//        }
//      }
...
//      //wait
//    }
//    assert(dp.internalQueue.peek(dp).isEmpty)
//  }
//
...
//    val logManager = new EmptyLogManagerImpl(null)
